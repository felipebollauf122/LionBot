...
async function listTenantCredentials(
  ...
  const out: ZuckpayCredential[] = [];
  for (const b of bots as Array<{ zuckpay_client_id: string | null; zuckpay_client_secret: string | null }>) {
    if (!b.zuckpay_client_id || !b.zuckpay_client_secret) continue;
...
export async function pollZuckpayPendingTransactions(db: SupabaseClient): Promise<void> {
  ...
  for (const [tenantId, txs] of byTenant) {
    ...
    for (const tx of txs) {
      try {
        ...
        if (!found) {
          console.log(
            `[zuckpay-poller] tx ${tx.external_id} not found em ${credentials.length} credencial(is) do tenant ${tenantId}`,
          );
          continue;
        }
