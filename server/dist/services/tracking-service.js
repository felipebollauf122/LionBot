...
function buildFbc(fbclid, clickTimeMs) {
    if (!fbclid)
        return "";
 ...
 */
async function loadClickContext(db, tid) {
    if (!tid)
        return {};
    const { data } = await db
 ...
 */
function buildExternalIds(lead) {
    const ids = new Set();
...
function buildFbUserData(lead, ctx) {
...
export class TrackingService {
    ...
    facebookCapi;
    utmify;
    constructor(db, facebookCapi, utmify) {
        this.db = db;
    ...
    async trackPurchase(params) {
        ...
        const leadWithDoc = { ...lead, document: lead.document || params.customerDocument };
        const userData = {
    ...
    async enrichGeo(tid) {
        try {
            ...
            console.error("[geo] enrichGeo falhou:", e.message);
        }
    }
    /**
     ...
     */
    static FUNNEL_CAPI_ENABLED = false;
    /**
    ...
    async trackCheckout(params) {
        ...
        if (!TrackingService.FUNNEL_CAPI_ENABLED)
            return;
