 ...
 */
export class ZuckPay implements PaymentGateway {
  private baseUrl = "https://www.zuckpay.com.br/conta/v3";
  ...
  async createPixPayment(params: CreatePixPaymentParams): Promise<PixPaymentResult> {
    if (!this.isConfigured()) {
      throw new Error(
        "ZuckPay não configurado. Vá em Configurações do bot e preencha o Client ID e o Client Secret.",
      );
    }
    ...
    const transactionId = String(
      ...
      (data as { transaction_id?: unknown }).transaction_id ??
    ...
    const pixCode = String(
      (data as { pix_code?: unknown }).pix_code ??
      (data as { qrcode?: unknown }).qrcode ??
      (data as { pixCode?: unknown }).pixCode ??
      (data as { brCode?: unknown }).brCode ??
      "",
    );
    const pixImage =
      String((data as { qrcode_image?: unknown }).qrcode_image ?? (data as { qrcodeImage?: unknown }).qrcodeImage ?? "") || null;
    const externalId = String(
      (data as { external_id?: unknown }).external_id ??
      (data as { external_id_client?: unknown }).external_id_client ??
      "",
    );
  ...
  async getPaymentStatus(transactionId: string): Promise<{ status: string } | null> {
    ...
    try {
      const response = await fetch(
        `${this.baseUrl}/pix/status?transactionId=${encodeURIComponent(transactionId)}`,
        {
          method: "GET",
