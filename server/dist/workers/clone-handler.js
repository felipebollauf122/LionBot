...
import { SourceReader, READ_THROTTLE_MS, } from "../services/mtproto/clone/source-reader.js";
import { ensureDestination } from "../services/mtproto/clone/dest-builder.js";
import { chooseStrategy, createPublisher, } from "../services/mtproto/clone/publish-router.js";
...
async function promoteBotTolerant(client, channelId, accessHash, botUsername) {
    try {
 ...
 * os dois caminhos.
 */
async function scheduleCloneResume(cloneJobId, seconds) {
...
export async function handleCloneRun(cloneJobId) {
    ...
    if (job.status !== "running" && job.status !== "waiting_flood") {
        console.log(`[clone] job ${cloneJobId} ignorado: status atual é '${job.status}' (esperado running/waiting_flood)`);
        return;
    }
    ...
    if (!claimed) {
        console.log(`[clone] job ${cloneJobId} não reivindicado (trava de outro worker ou status não roda mais), ignorando`);
        return;
    }
    try {
        ...
        try {
            ...
            if (wantsForum) {
                topicSync = await syncTopics({
                    ...
                    createDestTopic: (topicInput) => destClient.createForumTopic(dest.channelId, dest.accessHash, topicInput),
                    setClosed: (topicId, closed) => destClient.setForumTopicClosed(dest.channelId, dest.accessHash, topicId, closed),
                    setPinned: (topicId, pinned) => destClient.setForumTopicPinned(dest.channelId, dest.accessHash, topicId, pinned),
            ...
            // 2) Estratégia
            const linkReplaceConfigured = Boolean(job.link_replace_bot || job.link_replace_group || job.link_replace_channel);
            ...
            if (linkReplaceConfigured) {
                console.log(`[clone] job ${cloneJobId}: troca de link ativada — RPCs extras de resolução podem alongar o tempo total do clone`);
            }
            ...
            bot = new CompanionBot(botRow.token, CompanionBot.destChatIdFromChannelId(dest.channelId), botRow.session_string, { apiId: config.telegramApiId, apiHash: config.telegramApiHash });
            ...
            const linkReplace = linkReplaceConfigured
                ? {
                    classify: (identifier) => {
                        ...
                        return parsed ? client.classifyLink(parsed) : Promise.resolve("unknown");
                    },
                    values: {
            ...
            const runner = new CloneRunner({
                ...
                iterate: (since) => iterHistoryAscending(reader.historySource(), {
                ...
                persist: async (id, rows, cursor) => {
                    if (rows.length > 0) {
                        await supabase.from("clone_message_map").upsert(rows.map((r) => ({
                            job_id: id,
                ...
                loadIdMap: async (id) => {
                    ...
                    return (data ?? [])
                        ...
                        .map((r) => [Number(r.source_msg_id), Number(r.dest_msg_id)]);
                },
                ...
                setStatus: async (id, status, patch) => {
                    ...
                    const { error: writeError } = await supabase
                        ...
                        .eq("id", id);
                    if (writeError) {
                        console.error(`[clone] falha ao gravar status=${status} do job ${id}: ${writeError.message}`);
                    }
                },
            ...
            if (wantsForum && topicSync) {
                ...
                if (finalRow?.status === "completed") {
                    await finalizeTopics({
                        setClosed: (topicId, closed) => destClient.setForumTopicClosed(dest.channelId, dest.accessHash, topicId, closed),
                        setPinned: (topicId, pinned) => destClient.setForumTopicPinned(dest.channelId, dest.accessHash, topicId, pinned),
                    }, topicSync.topicMap, topicSync.sourceTopics).catch((err) => console.warn(`[clone] finalizeTopics falhou pro job ${cloneJobId} (não fatal):`, err));
                }
            }
        }
        catch (err) {
            ...
            if (wait !== null) {
                console.warn(`[clone] job ${cloneJobId} flood durante o setup (${wait}s), agendando retomada em vez de falhar`);
                await supabase
                    .from("clone_jobs")
            ...
            if (crossAccount)
                await destClient.disconnect().catch(() => { });
...
async function fail(cloneJobId, error) {
    ...
    const { error: writeError } = await supabase
        ...
        .eq("id", cloneJobId);
    if (writeError) {
        console.error(`[clone] falha ao marcar job ${cloneJobId} como failed: ${writeError.message}`);
    }
}
