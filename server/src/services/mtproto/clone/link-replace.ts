...
import type { TextReplacement, TextUrlReplacement } from "./link-rewrite.js";
...
function textValueFor(kind: PeerKind, values: LinkReplaceValues): string | undefined {
  if (kind === "bot") return values.botUsername ? `@${values.botUsername}` : undefined;
  if (kind === "group") return values.groupLink;
  if (kind === "channel") return values.channelLink;
  return undefined;
}

/** Valor de substituição pro campo .url de TextUrl/botão inline (scheme garantido). */
function urlValueFor(kind: PeerKind, values: LinkReplaceValues): string | undefined {
  if (kind === "bot") return values.botUsername ? ensureScheme(`t.me/${values.botUsername}`) : undefined;
  if (kind === "group") return values.groupLink ? ensureScheme(values.groupLink) : undefined;
  if (kind === "channel") return values.channelLink ? ensureScheme(values.channelLink) : undefined;
  return undefined;
}

type Candidate =
  | { source: "entity"; entity: Api.MessageEntityMention | Api.MessageEntityUrl; identifier: string }
  | { source: "textUrl"; entity: Api.MessageEntityTextUrl; identifier: string }
  | { source: "button"; link: InlineLink; identifier: string };
 ...
 * como retomável.
 */
export async function rewriteMessageLinks(
  input: RewriteMessageLinksInput,
  deps: LinkReplaceDeps,
  values: LinkReplaceValues,
): Promise<RewriteMessageLinksResult> {
  ...
  for (const { c, parsed } of parsedCandidates) {
    if (parsed) uniqueIdentifiers.add(c.identifier);
  }
  const classifications = new Map<string, PeerKind>();
  for (const identifier of uniqueIdentifiers) {
    classifications.set(identifier, await deps.classify(identifier));
  }

  const textReplacements: TextReplacement[] = [];
  const textUrlReplacements: TextUrlReplacement[] = [];
  const buttonReplacements = new Map<InlineLink, string>();
