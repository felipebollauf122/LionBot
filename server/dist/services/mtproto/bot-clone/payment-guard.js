 ...
 */
export function mapRawButton(btn) {
 ...
 */
function isGenericConfirmLabel(normalized) {
    const words = normalized.match(/[a-z]+/g) ?? [];
    if (words.length === 0 || words.length > 3)
        return false;
    return words.every((w) => GENERIC_CONFIRM_WORDS.has(w));
}
export const PAYMENT_KEYWORD_PATTERNS = [
    // PT
...
export const PAYMENT_DOMAIN_PATTERNS = [
 ...
 * revisão manual.
 */
export const PAYMENT_CONFIRMATION_PATTERNS = [
...
function matchesPaymentDomain(url) {
    try {
...
export function scanForPaymentConfirmation(text) {
 ...
 */
export function classifyButton(btn, messageText) {
    if (btn.kind === "buy")
        return { action: "skip", reason: "buy_button_native_payment" };
    if (btn.kind === "webview" || btn.kind === "simple_webview") {
        return { action: "skip", reason: "webview_miniapp_checkout" };
    }
    if (btn.kind === "game")
        return { action: "skip", reason: "game_unsupported" };
    if (btn.kind === "switch_inline")
        return { action: "skip", reason: "switch_inline_unsupported" };
    if (btn.kind === "url_auth")
        return { action: "skip", reason: "url_auth_unsupported" };
    if (btn.kind === "reply_keyboard_text") {
        return { action: "skip", reason: "reply_keyboard_text_unsupported" };
    }
    if (btn.kind === "unknown")
        return { action: "skip", reason: "unknown_button_kind" };
    if (btn.requiresPassword)
        return { action: "skip", reason: "requires_password_2fa_gated" };
