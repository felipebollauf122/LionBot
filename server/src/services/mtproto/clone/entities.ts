 ...
 */
export function toBotApiEntities(
...
function toBotApiEntity(e: Api.TypeMessageEntity): MessageEntity | null {
  const { offset, length } = e;

  if (e instanceof Api.MessageEntityBold) return { type: "bold", offset, length };
  if (e instanceof Api.MessageEntityItalic) return { type: "italic", offset, length };
  if (e instanceof Api.MessageEntityUnderline) return { type: "underline", offset, length };
  if (e instanceof Api.MessageEntityStrike) return { type: "strikethrough", offset, length };
  if (e instanceof Api.MessageEntitySpoiler) return { type: "spoiler", offset, length };
  if (e instanceof Api.MessageEntityCode) return { type: "code", offset, length };
  if (e instanceof Api.MessageEntityUrl) return { type: "url", offset, length };
  if (e instanceof Api.MessageEntityMention) return { type: "mention", offset, length };
  if (e instanceof Api.MessageEntityHashtag) return { type: "hashtag", offset, length };
  if (e instanceof Api.MessageEntityCashtag) return { type: "cashtag", offset, length };
  if (e instanceof Api.MessageEntityBotCommand) return { type: "bot_command", offset, length };
  if (e instanceof Api.MessageEntityEmail) return { type: "email", offset, length };
  if (e instanceof Api.MessageEntityPhone) return { type: "phone_number", offset, length };
  ...
  if (e instanceof Api.MessageEntityMentionName) return null;
  ...
  if (e instanceof Api.MessageEntityCustomEmoji) return null;
