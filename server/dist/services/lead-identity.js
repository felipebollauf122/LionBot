 ...
 */
export async function resolveTenantIdentity(db, tenantId, telegramUserId, botId, incoming) {
    const { data: existing } = await db
        ...
        .maybeSingle();
    const hasIncomingTracking = Boolean(incoming.tid ||
        incoming.fbclid ||
        incoming.utm_source ||
        incoming.utm_medium ||
    ...
    const fireAndForget = (p) => {
        Promise.resolve(p).then(() => { }, (err) => console.error("[lead-identity] write falhou:", err));
    };
    if (!existing) {
