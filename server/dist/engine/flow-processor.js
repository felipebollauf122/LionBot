...
export class FlowProcessor {
    ...
    delayQueue;
    executeDeps;
    constructor(db, leadService, delayQueue, deps) {
        this.db = db;
    ...
    async getNamedFlow(botId, name) {
        ...
        if (data) {
            ...
            flowByIdCache.set(data.id, data);
            return data;
        }
     ...
     */
    async executeFlow(flow, lead, telegram, chatId, startNodeId, isBlack, deleteAfterMinutes, persistPosition = true) {
        const deletionDelay = deleteAfterMinutes && deleteAfterMinutes > 0
            ? deleteAfterMinutes
            : isBlack
        ...
        while (currentNodeId && iterations < MAX_ITERATIONS) {
            ...
            if (result.blocked) {
                ...
                logEvent({ leadId: lead.id, botId: flow.bot_id, tenantId: lead.tenant_id }, "blocked", "Lead bloqueou o bot");
                return { blocked: true };
            }
            ...
            const isDelayPersist = !!result.delaySeconds && result.delaySeconds > 0 && !!result.nextNodeId && persistPosition;
            ...
            if (result.delaySeconds && result.delaySeconds > 0 && result.nextNodeId) {
                if (persistPosition) {
                    if (result.stateUpdates) {
                        await this.leadService.updatePositionAndState(lead.id, flow.id, result.nextNodeId, lead.state);
                    }
                    else {
     ...
     */
    async handleStartCommand(bot, lead, telegram, chatId, messageText, flowName) {
    ...
    async handleIncomingMessage(bot, lead, telegram, chatId, messageText) {
        ...
        if (lead.current_flow_id && lead.current_node_id) {
            ...
            if (flow) {
                const currentNode = flow.flow_data.nodes.find((n) => n.id === lead.current_node_id);
                if (currentNode?.type === "input") {
                    const variable = String(currentNode.data.variable ?? "");
                    const edges = flow.flow_data.edges.filter((e) => e.source === currentNode.id);
        ...
        for (const flow of flows) {
            ...
            if (flow.name === "_black_flow" || flow.name === "_visual_flow")
                continue;
    ...
    async handleCallbackQuery(bot, lead, telegram, chatId, callbackData) {
        const isBlack = lead.active_flow_name === "_black_flow";
