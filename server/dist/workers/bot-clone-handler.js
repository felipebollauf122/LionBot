...
import { BotExplorer, } from "../services/mtproto/bot-clone/explorer.js";
...
import { gramjsEntitiesToCaptured } from "../services/mtproto/bot-clone/entities-to-html.js";
import { mapRawButton, classifyButton } from "../services/mtproto/bot-clone/payment-guard.js";
import { buildFlowGraph } from "../services/mtproto/bot-clone/transcript-to-flow.js";
...
function mapStoredMessage(m) {
    return {
        ...
        mediaPublicUrl: m.media_public_url ?? null,
        buttons: (m.buttons ?? []).map(mapStoredButton),
    };
}
...
function classifyRawMedia(msg) {
    const mediaClassName = msg.media ? msg.media.className : null;
    const attrs = msg.media instanceof Api.MessageMediaDocument && msg.media.document instanceof Api.Document
    ...
    if (plan.kind === "poll")
        return { mediaKind: "poll", media: null, fileName: null };
...
function extractRawButtons(msg) {
 ...
 */
function captureBurstFromBot(rawClient, botPeerId) {
    return new Promise((resolve) => {
        ...
        const handler = async (event) => {
            ...
            if (!msg || !(msg instanceof Api.Message) || msg.out)
                return;
...
async function resolveTargetBot(client, username) {
    ...
    if (!user || !user.bot || !user.accessHash)
        return null;
    return { peerId: String(user.id), accessHash: String(user.accessHash) };
}
async function fail(cloneJobId, error) {
...
async function scheduleExploreResume(cloneJobId, seconds) {
...
async function loadExistingNodes(jobId) {
    ...
    return (data ?? []).map((row) => ({
        ...
        depth: row.depth,
        messages: (row.messages ?? []).map(mapStoredMessage),
    }));
}
// ---------------------------------------------------------------------------
// handleBotCloneExplore
// ---------------------------------------------------------------------------
export async function handleBotCloneExplore(cloneJobId) {
    ...
    try {
        ...
        try {
            ...
            let botPeerId = job.target_bot_peer_id;
            let botAccessHash = job.target_bot_access_hash;
            ...
            const deps = {
                ...
                captureBurst: () => captureBurstFromBot(client.raw, botPeerId),
                rehostMedia: (media, nodeIdHint, fileName) => downloadAndRehostMedia({ raw: client.raw, supabase }, { media, tenantId: job.tenant_id, jobId: cloneJobId, nodeIdHint, fileName, tmpDir, maxBytes: MAX_FILE_BYTES }),
                loadExistingNodes: () => loadExistingNodes(cloneJobId),
                persistNode: async (row) => {
                    const { data, error } = await supabase
                        ...
                        .single();
                    if (error || !data)
                        throw new Error(`persistNode falhou: ${error?.message}`);
                    ...
                    if (row.paymentConfirmationSuspected)
                        patch.suspected_payment_hit = true;
...
function hasPaymentBlockButton(m) {
    ...
    for (const raw of buttons) {
        ...
        if (decision.action === "skip" && PAYMENT_BLOCK_SKIP_REASONS.has(decision.reason))
            return true;
    }
    return false;
}
/** Agrupa mensagens (já em ordem cronológica) em "rajadas" por proximidade de horário. */
function groupIntoHistoricalBursts(messages) {
    const bursts = [];
    let current = [];
    let lastMs = 0;
 ...
 */
async function captureHistoricalRemarketing(client, botInputPeer, job, tmpDir) {
    const collected = [];
    ...
    if (collected.length >= HISTORICAL_REMARKETING_CAP) {
        console.warn(`[botclone.remarketing-history] job ${job.id}: histórico cortado em ${HISTORICAL_REMARKETING_CAP} mensagens (as mais recentes) — pode haver remarketing mais antigo não capturado`);
    }
    ...
    if (paymentBlockIdx === -1) {
        console.log(`[botclone.remarketing-history] job ${job.id}: nenhum bloco de pagamento encontrado no histórico retido — nada capturado como remarketing ainda`);
        return;
    }
    const afterFlow = chronological.slice(paymentBlockIdx + 1);
    if (afterFlow.length === 0)
        return;
    ...
    for (const burst of bursts) {
        ...
        for (let i = 0; i < burst.length; i++) {
            ...
            if (media && fileName) {
                mediaPublicUrl = await downloadAndRehostMedia({ raw: client.raw, supabase }, { media, tenantId: job.tenant_id, jobId: job.id, nodeIdHint: `remarketing_${m.id}`, fileName, tmpDir, maxBytes: MAX_FILE_BYTES }).catch((err) => {
                    console.warn(`[botclone.remarketing-history] rehost falhou pra msg ${m.id} (segue sem mídia):`, err);
                    return null;
                });
            }
            messages.push(serializeMessage({
                seq: i,
                rawMsgId: m.id,
                text: m.message || null,
    ...
    const totalMessages = (allRows ?? []).reduce((n, r) => n + (r.messages?.length ?? 0), 0);
...
// ---------------------------------------------------------------------------
export async function tickBotCloneStuckJobsWatchdog() {
    ...
    for (const row of stuck) {
        console.warn(`[botclone.watchdog] job ${row.id} travado em listening_remarketing (worker provavelmente caiu durante a leitura do histórico) — reiniciando pra retomar`);
        const { data: reset } = await supabase
            ...
            .maybeSingle();
        if (reset) {
            await enqueueMtproto({ kind: "botclone.explore", cloneJobId: row.id }).catch((err) => console.error(`[botclone.watchdog] reenqueue falhou pro job ${row.id}:`, err));
        }
    }
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function computeRemarketingIntervalMinutes(rows) {
    if (rows.length < 2)
        return 60;
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        if (gap > 0 && gap < minGapSec)
            minGapSec = gap;
    }
    if (!Number.isFinite(minGapSec))
        return 60;
    return Math.max(1, Math.round(minGapSec / 60));
}
export async function handleBotCloneBuildFlow(cloneJobId) {
    const { data: job } = await supabase.from("bot_clone_jobs").select("*").eq("id", cloneJobId).single();
    if (!job || job.status !== "building_flow")
        return;
    try {
        const { data: nodeRows } = await supabase.from("bot_clone_nodes").select("*").eq("job_id", cloneJobId);
        const capturedNodes = (nodeRows ?? []).map((row) => ({
            id: row.id,
            parentNodeId: row.parent_node_id,
            triggeredByButtonId: row.triggered_by_button_id,
            status: row.status,
        ...
        const syntheticRmNodes = (rmRows ?? []).map((r) => ({
            id: `rm_${r.id}`,
            parentNodeId: null,
            triggeredByButtonId: null,
            status: "explored",
            duplicateOfNodeId: null,
            messages: (r.messages ?? []).map(mapStoredMessage),
        }));
        ...
        const priceMap = priceCandidates.size > 0
        ...
        const { data: flowRow, error: flowErr } = await supabase
            ...
            .single();
        if (flowErr || !flowRow)
            throw new Error(`insert flows falhou: ${flowErr?.message}`);
        let remarketingConfigId = null;
        if (rmRows && rmRows.length > 0) {
            const { data: cfgRow, error: cfgErr } = await supabase
                ...
                .single();
            if (cfgErr || !cfgRow)
                throw new Error(`insert remarketing_configs falhou: ${cfgErr?.message}`);
            remarketingConfigId = cfgRow.id;
