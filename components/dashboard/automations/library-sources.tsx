...
const labels:Record<string,string>={pending:"Aguardando",importing:"Importando histórico",watching:"Escutando novas postagens",paused:"Pausada",failed:"Precisa de atenção",completed:"Histórico importado"};
export function LibrarySources({libraryId,sources,dialogs}:{libraryId:string;sources:LibrarySource[];dialogs:LibraryDialog[]}){
 ...
 return <div className="space-y-6">
  ...
  {!sources.length?<p className="py-8 text-sm text-(--text-secondary)">Adicione sua primeira origem. Depois, configure as regras e ative a automação.</p>:<div className="divide-y divide-(--border-default)">{sources.map(s=><div key={s.id} className="flex flex-wrap items-start justify-between gap-4 py-5">
   <div className="min-w-0"><h3 className="font-medium text-foreground">{dialogs.find(d=>d.id===s.source_dialog_id)?.title||"Origem indisponível — sincronize a conta"}</h3><p className="mt-1 text-sm text-(--text-secondary)">{labels[s.status]||s.status} · {s.imported_count} mensagens no acervo</p><p className="mt-1 text-xs text-(--text-muted)">{s.import_history?"Histórico completo":""}{s.import_history&&s.watch?" + ":""}{s.watch?"Novas postagens":""}</p>{s.last_error&&<p className="mt-2 max-w-2xl break-words text-sm text-(--red)">{s.last_error}</p>}</div>
