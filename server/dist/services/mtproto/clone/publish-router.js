...
export function chooseStrategy(input) {
    ...
    if (input.copyReplies)
        return "download";
    if (input.linkReplaceConfigured)
        return "download";
    if (input.sourceHasNoForwards)
        return "download";
    return input.requested === "download" ? "download" : "batch";
}
export function routeGroup(input) {
 ...
 */
function resolveDestTopicId(topicMap, group) {
    if (!topicMap)
        return undefined; // job sem fórum: nunca toca message_thread_id/topMsgId
    const sourceTopicId = group[0].topicId;
    if (sourceTopicId === null)
        return undefined; // General, ou mensagem fora de tópico
 ...
 */
export function createPublisher(ctx) {
    return async (group, replyToDestId) => {
        const destTopicId = resolveDestTopicId(ctx.topicMap, group);
        const raws = group.map((m) => m.raw);
        const plans = raws.map((r) => planForMessage(SourceReader.mediaPlanInput(r, ctx.copyPolls)));
        ...
        if (decision.mode === "forward") {
            ...
            const updates = await ctx.reader.forwardBatch(ctx.destChannelId, ctx.destAccessHash, [...ids].sort((a, b) => a - b), destTopicId);
            const destIds = extractNewMessageIds(updates);
            let cursor = 0;
            return plans.map((p, i) => {
                if (skip.has(i)) {
                    return { status: "skipped", reason: p.kind === "skip" ? p.reason : "skip" };
                }
                const destMsgId = destIds[cursor++];
                return destMsgId
                    ? { status: "copied", destMsgId }
                    : { status: "failed", reason: "sem_id_no_retorno" };
            });
        }
        ...
        try {
            if (decision.mode === "album") {
                ...
                for (let i = 0; i < raws.length; i++) {
                    ...
                    if (ctx.linkReplace) {
                        const rewritten = await rewriteMessageLinks({ message: raws[i].message, entities: raws[i].entities, inlineLinks: undefined }, { classify: ctx.linkReplace.classify }, ctx.linkReplace.values);
                ...
                if (ok.length === 0) {
                    return plans.map(() => ({ status: "skipped", reason: "file_too_large" }));
                }
                ...
                if (tooLarge.size > 0) {
                    const degraded = new Array(raws.length);
                    let replyUsed = false;
                ...
                if (destIds.length !== items.length) {
                    return items.map((_, i) => i < destIds.length
                        ? { status: "copied", destMsgId: destIds[i] }
                        : { status: "failed", reason: "album_id_count_mismatch" });
                }
                return destIds.map((destMsgId) => ({ status: "copied", destMsgId }));
            }
            for (let i = 0; i < raws.length; i++) {
                const raw = raws[i];
                const plan = plans[i];
                ...
                if (ctx.linkReplace && plan.kind !== "poll") {
                    const rewritten = await rewriteMessageLinks({ message: raw.message, entities: raw.entities, inlineLinks: opts.inlineLinks }, { classify: ctx.linkReplace.classify }, ctx.linkReplace.values);
                    text = rewritten.text;
...
export function extractNewMessageIds(updates) {
