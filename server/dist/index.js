...
import { botCache, flowCache } from "./cache.js";
const app = express();
...
app.post("/api/bots/:botId/evpay-reconcile", async (req, res) => {
    try {
        const { botId } = req.params;
        const hours = Math.min(168, Math.max(1, Number(req.query.hours ?? 72) || 72));
        const { data: bot } = await supabase
            .from("bots")
...
app.post("/api/bots/:botId/send-message", async (req, res) => {
    try {
        ...
        logOutgoing({ leadId: lead.id, botId: lead.bot_id, tenantId: lead.tenant_id }, trimmed, sent?.message_id);
