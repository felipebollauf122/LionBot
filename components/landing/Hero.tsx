...
export default function Hero() {
  return (
    <section className="relative min-h-screen overflow-hidden bg-(--bg-root) flex flex-col scanlines">
      {/* Synthwave horizon grid — perspective floor */}
      ...
      <div className="relative z-10 flex-1 flex items-center">
        <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pt-32 pb-24">
          {/* Live badge */}
          <div className="animate-up inline-flex items-center gap-2.5 mb-10 border border-(--cyan)/25 bg-(--cyan)/[0.06] rounded-full px-4 py-1.5">
          ...
          {/* Headline — massive, left-aligned, chromatic */}
          <h1
            className="mb-8 leading-[0.92] tracking-tight"
            style={{ fontFamily: 'var(--font-display)' }}
          >
            <span className="animate-up-1 block text-[clamp(3rem,7.5vw,5.5rem)] font-bold text-(--text-primary)">
              Você ainda
            </span>
            <span className="animate-up-2 block text-[clamp(3rem,7.5vw,5.5rem)] font-bold text-(--text-ghost)">
              vende no
            </span>
            <span className="animate-up-3 block text-[clamp(3rem,7.5vw,5.5rem)] font-bold text-(--text-primary)">
              Telegram
            </span>
            <span className="animate-up-4 block text-[clamp(3rem,7.5vw,5.5rem)] font-bold gradient-text chromatic">
              na mão?
            </span>
          </h1>
          ...
          <div className="max-w-xl">
            <p className="animate-up-4 text-lg text-(--text-secondary) leading-relaxed mb-4">
              PIX manual. Follow-up no braço. Sem rastreamento. Cada venda que
              você fecha custou trabalho que deveria ser automático.
            </p>
            <p className="animate-up-5 text-base text-(--text-secondary) leading-relaxed mb-10">
              O LionBot coloca seu Telegram em{' '}
              <strong className="text-(--text-primary) font-semibold">modo piloto automático</strong>{' '}
              — funil, cobrança, recuperação e tracking rodando 24h sem você tocar em nada.
            </p>

            {/* CTA row */}
            <div className="animate-up-5 flex flex-col sm:flex-row gap-4 items-start">
          ...
          <div className="grid grid-cols-3 divide-x divide-(--border-subtle)">
            {stats.map((s) => (
              <div key={s.label} className="py-5 sm:py-6 px-4 sm:px-8 text-center">
                <p className="text-xl sm:text-2xl text-(--text-primary) mb-0.5 stat-value">
                  {s.value}
                </p>
      ...
      {/* Floating lion crest — top right, subtle */}
