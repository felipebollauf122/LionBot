...
export async function handlePaymentBundleNode(ctx, db, _gateway, _baseWebhookUrl) {
    ...
    const productButtons = items.map((item) => {
        ...
        if (product.button_style)
            btn.style = product.button_style;
        return btn;
    });
    ...
    if (botForTracking) {
        ...
        trackingSvc.trackViewOffer({
            ...
            contentName: typedBundle.ghost_name?.trim() ||
...
export async function handleProductPaymentCallback(ctx, db, gateway, baseWebhookUrl, productId, gatewayKind = "sigilopay", paymentButtonId) {
    ...
    // quando ghost vazio.
    const { productLabelForExternal, productDescriptionForExternal } = await import("../../services/external-product-label.js");
    ...
    console.log(`[payment] gatewayName="${gatewayName}" gatewayDescription="${gatewayDescription ?? ""}" (ghost_name set=${!!typedProduct.ghost_name})`);
    ...
    console.log(`[gateway-call] kind=${gatewayKind} productId=${typedProduct.id} ghost_name="${typedProduct.ghost_name ?? ""}" ghost_description="${typedProduct.ghost_description ?? ""}" → sending:`, JSON.stringify(gatewayParams, null, 2));
    let payment;
    try {
