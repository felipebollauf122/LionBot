 ...
 * formatação.
 */
export function gramjsEntitiesToCaptured(entities) {
    if (!entities)
        return [];
    const out = [];
    for (const e of entities) {
        ...
        if (e instanceof Api.MessageEntityBold)
            out.push({ type: "bold", offset, length });
        else if (e instanceof Api.MessageEntityItalic)
            out.push({ type: "italic", offset, length });
        else if (e instanceof Api.MessageEntityUnderline)
            out.push({ type: "underline", offset, length });
        else if (e instanceof Api.MessageEntityStrike)
            out.push({ type: "strike", offset, length });
        else if (e instanceof Api.MessageEntitySpoiler)
            out.push({ type: "spoiler", offset, length });
        else if (e instanceof Api.MessageEntityCode)
            out.push({ type: "code", offset, length });
...
function wrapEntity(entity, inner) {
    switch (entity.type) {
        case "bold":
            return `<b>${inner}</b>`;
        case "italic":
...
function unparseSegment(text, entities, segmentOffset, segmentLength) {
    ...
    for (let i = 0; i < entities.length; i++) {
        ...
        if (entity.offset >= segmentOffset + segmentLength)
            break;
    ...
    if (lastOffset < text.length)
        out.push(escapeHtml(text.slice(lastOffset)));
    return out.join("");
}
