...
export async function addDelayedJob(data, delaySeconds) {
...
export const paymentTimeoutQueue = new Queue("payment-timeout", {
    connection,
});
export async function addPaymentTimeoutJob(data, delaySeconds) {
...
export const purchaseEmailTimeoutQueue = new Queue("purchase-email-timeout", {
    connection,
});
export async function addPurchaseEmailTimeoutJob(data, delaySeconds) {
 ...
 */
async function processMessageDeletions() {
...
export function startWorkers() {
    ...
    new Worker("delayed-messages", async (job) => {
        ...
        let flow = flowByIdCache.get(flowId);
        if (!flow) {
        ...
        const processor = new FlowProcessor(supabase, leadService, { addDelayedJob }, { gateway, gatewayKind, baseWebhookUrl: config.baseWebhookUrl });
        const isBlack = lead.active_flow_name === "_black_flow";
        await processor.executeFlow(flow, lead, telegram, chatId, nodeId, isBlack);
    }, {
        connection,
        concurrency: 10,
    ...
    new Worker("purchase-email-timeout", async (job) => {
        ...
        if (!tx)
            return;
        let bot = botCache.get(tx.bot_id);
        if (!bot) {
    ...
    setInterval(() => {
        processMessageDeletions().catch((err) => console.error("[black-delete] Error:", err));
    }, 30_000);
    // Run once at startup to catch any overdue deletions
    processMessageDeletions().catch((err) => console.error("[black-delete] Startup error:", err));
    ...
    async function tickMtprotoAutoSync() {
        if (mtprotoSyncRunning)
            return;
    ...
    async function tickMtprotoHealth() {
        if (mtprotoHealthRunning)
            return;
        ...
        try {
            ...
            for (const acc of accounts) {
                if (!acc.session_string)
                    continue;
                const client = new MtprotoClient(config.telegramApiId, config.telegramApiHash, acc.session_string);
                try {
    ...
    async function cleanupInboxMessages() {
        ...
        if (count && count > 0)
            console.log(`[inbox-cleanup] removed ${count} msgs older than 7d`);
    }
    ...
    async function cleanupLeadMessages() {
        ...
        if (count && count > 0)
            console.log(`[lead-messages-cleanup] removed ${count} msgs older than 30d`);
    }
    ...
    async function tickChannelMonitor() {
        if (channelMonitorRunning)
            return;
