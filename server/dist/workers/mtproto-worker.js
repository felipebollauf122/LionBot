...
const inProgressSyncs = new Set();
async function getOrCreateClient(accountId, sessionString) {
...
async function sweepIdleClients(force = false) {
    const now = Date.now();
...
export async function shutdownMtprotoClients() {
...
async function notifyLoginBot(accountId, kind, errorMsg) {
    try {
        const handler = await import("../webhook/mtproto-login-handler.js");
        if (kind === "code_sent")
            await handler.notifyLoginCodeSent(accountId);
        else if (kind === "needs_password")
            await handler.notifyLoginNeedsPassword(accountId);
        else if (kind === "success")
            await handler.notifyLoginSuccess(accountId);
        else if (kind === "error")
            await handler.notifyLoginError(accountId, errorMsg ?? "unknown");
    }
    catch (err) {
        console.error(`[mtproto] notifyLoginBot(${kind}) failed for ${accountId}:`, err);
    }
}
async function handleRequestCode(accountId, phoneNumber) {
...
async function handleSignIn(accountId, phoneNumber, code) {
    ...
    if (!client)
        throw new Error("client not live — re-solicite o código");
    try {
        ...
        if (result.ok) {
            ...
            await enqueueMtproto({ kind: "account.sync-dialogs", accountId }).catch((err) => console.error(`[mtproto] failed to enqueue initial sync for ${accountId}:`, err));
        ...
        if (/PHONE_CODE_EXPIRED|PHONE_CODE_INVALID|PHONE_CODE_EMPTY/i.test(msg)) {
            ...
            await enqueueMtproto({ kind: "auth.request-code", accountId, phoneNumber }).catch((e) => console.error("[mtproto] reenqueue request-code failed:", e));
...
async function handleSubmitPassword(accountId, password) {
    const client = liveClients.get(accountId)?.client;
    if (!client)
        throw new Error("client not live — re-solicite o código");
    try {
...
async function handleSyncDialogs(accountId) {
...
async function doSyncDialogs(accountId) {
    ...
    await addAccountToActiveGlobalCampaigns(accountId).catch((err) => console.error(`[mtproto.hot-add] account ${accountId} falhou:`, err));
}
/**
 ...
 */
async function addAccountToActiveGlobalCampaigns(accountId) {
    const { data: account } = await supabase
        .from("mtproto_accounts")
    ...
    for (const camp of campaigns) {
        ...
        await supabase
            ...
            .eq("id", camp.id);
        console.log(`[mtproto.hot-add] ${rows.length} targets da conta ${accountId} adicionados à campanha ${camp.id} (status=${camp.status})`);
        ...
        if (camp.status === "running") {
            await enqueueMtproto({ kind: "campaign.run", campaignId: camp.id }).catch((err) => console.error(`[mtproto.hot-add] enqueue campaign.run falhou:`, err));
        }
    }
}
async function refreshGlobalCampaignTargets(campaignId, tenantId) {
...
async function handleCampaignRun(campaignId) {
...
async function runCampaignInner(campaignId, campaign) {
    ...
    if (campaign.is_global) {
        ...
        if ((alreadySent ?? 0) === 0) {
            ...
            console.log(`[runner] campanha ${campaignId}: pulando refresh (já tem ${alreadySent} sent — re-entrada)`);
        }
    }
    ...
    async function loadAccountsAndPool(pool) {
        ...
        pool.load(accountsSnapshot.map((a) => ({
            id: a.id,
    ...
    async function fetchPendingTargets() {
        ...
        return (targets ?? []).map((t) => {
            ...
            const dialog = t.mtproto_dialogs;
            if (dialog) {
                row.dialog = {
    ...
    const runner = new CampaignRunner(pool, {
        sendMessage: async (accountId, target, text) => {
            ...
            if (target.dialog) {
                ...
                await client.sendMessageToPeer(target.dialog.peerId, target.dialog.peerType, target.dialog.peerAccessHash, text);
            }
            else {
...
export function startMtprotoWorker() {
