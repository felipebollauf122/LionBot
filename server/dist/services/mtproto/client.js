...
export class MtprotoClient {
    ...
    async sendCode(phoneNumber) {
        await this.connect();
        const result = await this.client.sendCode({ apiId: this.apiId, apiHash: this.apiHash }, phoneNumber);
    ...
    async signInWithPassword(password) {
        await this.client.signInUser({ apiId: this.apiId, apiHash: this.apiHash }, {
            phoneNumber: async () => "",
            phoneCode: async () => "",
    ...
    async sendMessage(target, targetType, text) {
        ...
        if (!user) {
            const imported = await this.client.invoke(new Api.contacts.ImportContacts({
                contacts: [
            ...
            if (!user)
                throw new Error("PHONE_NOT_ON_TELEGRAM");
            this.phoneUserCache.set(target, user);
        }
        await this.client.sendMessage(user, { message: text });
    }
    /**
     ...
     */
    async sendMessageToPeer(peerId, peerType, peerAccessHash, text) {
        await this.connect();
        let inputPeer;
        if (peerType === "user") {
            if (!peerAccessHash)
                throw new Error("USER_PEER_MISSING_ACCESS_HASH");
        ...
        await this.client.sendMessage(inputPeer, { message: text });
    }
    /**
    ...
    async listDialogs() {
        ...
        while (hasMore && iter < maxIterations) {
            ...
            let result;
            try {
                result = await this.client.invoke(new Api.messages.GetDialogs({
                    offsetDate,
                    offsetId,
                    offsetPeer,
                    limit: dialogLimit,
            ...
            if (!(result instanceof Api.messages.Dialogs) &&
                !(result instanceof Api.messages.DialogsSlice)) {
                break;
            }
            const dialogList = result.dialogs;
            const userList = result.users;
            const chatList = result.chats;
            ...
            for (const dialog of dialogList) {
                ...
                if (dialogPeer instanceof Api.PeerUser) {
                    ...
                    if (!c || c instanceof Api.ChatForbidden || c instanceof Api.ChannelForbidden)
                        continue;
                    if (!(c instanceof Api.Chat))
                        continue;
            ...
            if (result instanceof Api.messages.DialogsSlice &&
                ...
                if (lastDialog instanceof Api.Dialog) {
                    ...
                    const lastMessage = messageList.find((m) => (m instanceof Api.Message || m instanceof Api.MessageService) &&
                        m.id === lastMessageId);
     ...
     */
    async getTelegramOfficialHistory(limit = 50) {
        await this.connect();
        ...
        const messages = result instanceof Api.messages.Messages ||
    ...
    startInboxListener(onMessage) {
        ...
        const handler = async (event) => {
            ...
            if (fromId !== TELEGRAM_OFFICIAL_USER_ID)
                return;
            await onMessage({
                tgMessageId: msg.id,
                fromPeerId: TELEGRAM_OFFICIAL_USER_ID,
     ...
     */
    async getChannelStatus(channelId, accessHash) {
        try {
            await this.connect();
            ...
            const result = await this.client.invoke(new Api.channels.GetChannels({ id: [inputChannel] }));
            const chats = result instanceof Api.messages.Chats || result instanceof Api.messages.ChatsSlice
                ? result.chats
                : [];
    ...
    async createChannel(title, about) {
        ...
        if (!(result instanceof Api.Updates) &&
            ...
            throw new Error(`createChannel: unexpected return type ${result.className}`);
        }
        const channel = result.chats.find((c) => c instanceof Api.Channel);
     ...
     */
    async sendTextToChannel(channelId, accessHash, text) {
        await this.connect();
        const peer = new Api.InputPeerChannel({
            channelId: bigInt(channelId),
            accessHash: bigInt(accessHash),
     ...
     */
    async exportChannelInvite(channelId, accessHash) {
        await this.connect();
        const peer = new Api.InputPeerChannel({
            channelId: bigInt(channelId),
            accessHash: bigInt(accessHash),
        });
        const result = await this.client.invoke(new Api.messages.ExportChatInvite({ peer }));
     ...
     */
    async setChannelPhoto(channelId, accessHash, photoBuffer, fileName = "channel_photo.jpg") {
        await this.connect();
     ...
     */
    async setChannelReactions(channelId, accessHash, enabled) {
        await this.connect();
        const peer = new Api.InputPeerChannel({
            channelId: bigInt(channelId),
            accessHash: bigInt(accessHash),
        });
        await this.client.invoke(new Api.messages.SetChatAvailableReactions({
            peer,
     ...
     */
    async setChannelProtectContent(channelId, accessHash, enabled) {
        await this.connect();
        const channel = new Api.InputChannel({
            channelId: bigInt(channelId),
            accessHash: bigInt(accessHash),
        });
        await this.client.invoke(new Api.messages.ToggleNoForwards({
