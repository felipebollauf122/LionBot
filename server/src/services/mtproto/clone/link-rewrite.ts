...
function withOffsetLength<T extends Api.TypeMessageEntity>(
  entity: T,
  offset: number,
  length: number,
): T {
  ...
  ) as T & { offset: number; length: number };
  clone.offset = offset;
  clone.length = length;
  return clone;
}
...
export function applyTextReplacements(
...
): { text: string; entities: Api.TypeMessageEntity[] | undefined } {
  if (replacements.length === 0) return { text, entities };
  ...
  function mapPos(p: number): number {
    let delta = 0;
    for (const r of accepted) {
  ...
  const outEntities: Api.TypeMessageEntity[] = [];
 ...
 */
export function applyTextUrlReplacements(
  entities: Api.TypeMessageEntity[] | undefined,
  replacements: TextUrlReplacement[],
): Api.TypeMessageEntity[] | undefined {
  if (replacements.length === 0 || !entities) return entities;
  return entities.map((e) => {
    if (!(e instanceof Api.MessageEntityTextUrl)) return e;
