...
export async function syncTopics(
  ...
  const topicMap = new Map<number, number>([[1, 1]]);
  ...
  for (const topic of sourceTopics) {
    if (topic.id === 1) continue; // General: nunca recriado
    if (topicMap.has(topic.id)) continue; // já copiado numa execução anterior
    ...
    try {
      ...
      if (extractWaitSeconds(err) !== null) throw err; // flood sobe pro setup do clone-handler
...
export async function finalizeTopics(
  deps: Pick<TopicSyncDeps, "setClosed" | "setPinned">,
  topicMap: Map<number, number>,
  sourceTopics: SourceTopic[],
): Promise<void> {
  for (const topic of sourceTopics) {
    const destTopicId = topicMap.get(topic.id);
    if (destTopicId === undefined || topic.id === 1) continue; // não criado, ou é o General
    if (topic.closed) {
      await deps
