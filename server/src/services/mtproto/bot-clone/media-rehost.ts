...
async function ensureMediaBucket(supabase: SupabaseClient): Promise<void> {
...
export interface DownloadAndRehostInput {
  media: unknown; // Api.TypeMessageMedia
  tenantId: string;
  jobId: string;
  nodeIdHint: string;
  fileName: string;
...
export async function downloadAndRehostMedia(
  ...
  try {
    ...
    if (error) throw new Error(`upload pro Storage falhou: ${error.message}`);
